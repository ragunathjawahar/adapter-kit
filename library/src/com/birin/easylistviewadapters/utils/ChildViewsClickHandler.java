 ...
 * current row position, when click event occurs this row position data & event
 * id is retrieved from view tag & passed to client in callback.
 * 
 * Whenever {@link getNewRow()} method is called ,a view holder is returned &
 * user registers few children of row for clicking by calling
 ...
 * views with current row position if they were registered for click events so
 * that this position can be used to identify when click event happens.
 * 
 */
public final class ChildViewsClickHandler {
	 ...
	 * view present inside a row should register for these events for parent row
	 * view click events use
	 * {@link ListView#setOnItemClickListener(android.widget.AdapterView.OnItemClickListener)}
	 * 
	 * @param childView
	 *            the child view (present within RowView) which needs to receive
	 *            click event.
	 * @param clickEventId
